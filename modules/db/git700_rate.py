...
class Cassandra_git700_rate(PgDbCassandra):
    ...
    def sentiment_upsert_sentence(self, id_www_source, id_project, id_data_text, id_token_sentence, id_rating_type, autocommit = True):
        return (''' SELECT * FROM git700_rate.upsert_sentence($1, $2, $3, $4, $5) ''', 
                ["dmn.git_pk","dmn.git_pk","dmn.git_pk","dmn.git_pk","dmn.git_pk"])

    @wrap.execute_with_query_plan_0
    def sentiment_upsert_text(self, id_www_source, id_project, id_data_text, id_rating_type, autocommit = True):
